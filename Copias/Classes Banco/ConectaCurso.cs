    ...
    class ConectaCurso
    {
        ...
        public void cadastro(Curso us)
        {
            try
            {
                ...
                conexao.Open();
                string inserir = "INSERT INTO curso(nome, modulo, situacao, coord, coord2, tipo, periodo, num_aulas)VALUES('" + us.Nome + "','" + us.Modulo + "','" + us.Situacao +"','" + us.Coordenador + "','" + us.Coordenador2 + "','" + us.Tipo + "','" + us.Periodo + "','" + us.NumAulas + "')";
        ...
        public void atualizar(Curso us)
        {
            ...
            try
            {
                conexao = new MySqlConnection(Endereco());
                conexao.Open();
                string alterar = "UPDATE curso SET situacao ='" + us.Situacao + "', modulo = '" + us.Modulo + "', coord = '" + us.Coordenador +"', coord2 = '" + us.Coordenador2 + "', tipo='" + us.Tipo + "', periodo='" + us.Periodo + "', num_aulas='" + us.NumAulas + "' WHERE nome = '" + us.Nome + "'AND modulo = '" + us.Modulo + "'";
        ...
        public DataTable Curso()
        {
            conexao = new MySqlConnection(Endereco());
            conexao.Open();
            string vSQL = "SELECT modulo, nome, tipo, periodo, num_aulas FROM curso WHERE situacao = 'Ativo' ORDER BY tipo, cod";
        ...
        public DataTable CarregaGridCurso(Curso us)
        {
            ...
            if (us.Situacao == "Ativo" || us.Situacao == "Inativo")
            {
                conexao = new MySqlConnection(Endereco());
                conexao.Open();
                string vSQL = "SELECT cod, nome, modulo, situacao, coord, coord2, tipo, periodo, num_aulas FROM curso WHERE situacao = '" + us.Situacao + "'ORDER BY cod";
                MySqlDataAdapter vDataAdapter = new MySqlDataAdapter(vSQL, conexao);
                vDataAdapter.Fill(vTable);
                conexao.Close();
            }
            else if (us.Situacao == "Todos")
            {
                conexao = new MySqlConnection(Endereco());
                conexao.Open();
                string vSQL = "SELECT cod, nome, modulo, situacao, coord, coord2, tipo, periodo, num_aulas FROM curso ORDER BY cod";
                MySqlDataAdapter vDataAdapter = new MySqlDataAdapter(vSQL, conexao);              
                vDataAdapter.Fill(vTable);
                conexao.Close();
            }
